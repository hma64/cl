  ...
  addProductForm.addEventListener('submit', async (ev) => {
    ...
    try {
      if (id) {
        ...
        addProductAlert.className = 'admin-alert success';
      } else {
        await addDoc(collection(db, 'products'), payload);
        resetProductForm();
        await loadProducts();
        addProductAlert.textContent = 'Produit ajouté avec succès.';
        addProductAlert.className = 'admin-alert success';
      }
    } catch (err) {
      console.error(err);
      addProductAlert.textContent = id ? 'Erreur lors de la mise à jour.' : 'Erreur lors de l\'ajout.';
      addProductAlert.className = 'admin-alert error';
    }
    setTimeout(() => {
      if (addProductAlert) { addProductAlert.textContent = ''; addProductAlert.className = 'admin-alert'; }
    }, 2200);
  });
}
