...
import { getWishlist, getWishlistCount } from "./wishlist.js";
...
export function injectNav({ page = "home", cartCountEl } = {}) {
  ...
  wrapper.innerHTML = `
    ...
    <aside class="slide-menu" id="slideMenu" aria-label="Menu de navigation">
      ...
      <ul class="slide-menu-links" id="slideMenuLinks">
        ...
        <hr class="slide-menu-divider" />
        <li>
          <button type="button" class="slide-menu-wishlist-btn" id="slideMenuWishlistBtn">
            <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
            قائمتي (<span id="slideMenuWishlistCount">0</span>)
          </button>
        </li>
        <hr class="slide-menu-divider" />
        <li>
  ...
  const slideMenuWishlistBtn = document.getElementById("slideMenuWishlistBtn");
  const slideMenuWishlistCount = document.getElementById("slideMenuWishlistCount");
  
  if (slideMenuWishlistBtn) {
    slideMenuWishlistBtn.addEventListener("click", () => {
      closeMenu();
...
async function loadCategories(currentPage) {
  ...
  function renderCategories(cats) {
    if (catContainer) {
      const activeClass = currentPage === 'home' ? 'class="active"' : '';
      catContainer.innerHTML = `<a href="index.html" ${activeClass}>الرئيسية</a>` + cats.map(c =>
...
function updateWishlistCount() {
  ...
  const countEl = document.getElementById("slideMenuWishlistCount");
  if (countEl) {
