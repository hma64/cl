...
// The nav injects #openCartBtn — use delegated listener so clicks work even before
// the element is created (fixes 'panier' button sometimes stuck on first click)
document.addEventListener('click', (ev) => {
  const btn = ev.target.closest && ev.target.closest('#openCartBtn');
  if (btn) {
    // Prevent double-activation when element already has its own handler
    ev.preventDefault();
    openCart();
  }
});
