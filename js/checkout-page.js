...
const continueShoppingBtn = typeof document !== 'undefined' ? document.getElementById('continueShoppingBtn') : null;
const closeModalBtn = typeof document !== 'undefined' ? document.getElementById('closeModalBtn') : null;
...
function renderReview() {
  ...
  cart.forEach((item) => {
    ...
    const sizeText = `${escapeHtml(item.color)}`;
    row.innerHTML = `
...
checkoutForm.addEventListener("submit", async (e) => {
  ...
  const subject = `Bon de commande Cristian Lay — ${firstName} ${lastName} — ${orderedAt}`;
  const orderData = {
    firstName,
    lastName,
    phone,
  ...
  try {
    ...
    if (!ok || data.error) {
      if (saved) {
        ...
        setCart([]);
        // show modal indicating success but email failed
    ...
    // Successful send — show professional Arabic modal offering to continue shopping
    setCart([]);
    showOrderSuccessModal();
  } catch (err) {
    console.error(err);
...
if (orderModal) {
  orderModal.addEventListener('click', (ev) => {
    if (ev.target === orderModal) hideOrderSuccessModal();
  });
}
